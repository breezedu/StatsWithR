...
ggplot(data = sample_means50price, aes(x = x_bar)) +
...
sample_means150price <- ames %>%
                          rep_sample_n(size = 150, reps = 15000, replace = TRUE) %>%
                          summarise(x_bar = mean(price))

ggplot(data = sample_means150price, aes(x = x_bar)) +
  geom_histogram(binwidth = 10)
...
# type your code for this Exercise here, and Run Document 
sample_means15price <- ames %>%
                          rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>%
                          summarise(x_bar = mean(price))

ggplot(data = sample_means15price, aes(x = x_bar)) +
  geom_histogram(binwidth = 10)
...
# compare the SE between two samplings
sample_means50price <- ames %>%
                          rep_sample_n(size = 50, reps = 15000, replace = TRUE) %>%
                          summarise(x_bar = mean(price))
sample_means150price <- ames %>%
...
SE1 <- sd(sample_means150price$x_bar)/sqrt( length(sample_means150price$x_bar) )

SE2 <- sd(sample_means50price$x_bar) / sqrt(length(sample_means50price$x_bar) )
